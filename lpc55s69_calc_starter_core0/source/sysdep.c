...
void sys_out_mode (int mode)
{
	...
	switch (mode) {
	case CC_OUTPUT:
		uart_puts(USART0, "\x1b[32m");  // Vert
		break;

	case CC_WARN:
		uart_puts(USART0, "\x1b[33m");  // Jaune
		break;

	case CC_ERROR:
		uart_puts(USART0, "\x1b[31m");  // Rouge
		break;

	case CC_EDIT:
		uart_puts(USART0, "\x1b[0m");  // magenta
		uart_puts(USART0,"> ");
		break;

	case CC_FEDIT:
		uart_puts(USART0, "\x1b[36m");  // Cyan
		uart_puts(USART0,"$ ");
		break;

	default:
		uart_puts(USART0, "\x1b[0m");  // Blanc
		break;
	}
}
...
int sys_wait_key (scan_t *scan)
{
	char buf_vt100[5] = {0};
		...
		/* scan is specific to calc but I have to throw back the sequence
		...
		case '\n':
			...
			*scan=enter;
			break;
		case 27:
			/* Send the ANSI escape sequence (VT100) back */
			while (c)
			{
				c=uart_getc(USART0);
				buf_vt100[i++]=c;
				if (strcmp(buf_vt100, "[A") == 0)
				{
					*scan = cursor_up;
					break;
				}
				if (strcmp(buf_vt100, "[B") == 0)
				{
					*scan = cursor_down;
					break;
				}
				if (strcmp(buf_vt100, "[C") == 0)
				{
					*scan = cursor_right;
					break;
				}
				if (strcmp(buf_vt100, "[D") == 0)
				{
					*scan = cursor_left;
					break;
				}
				if (strcmp(buf_vt100, "[3~") == 0)
				{
					*scan = delete;
					break;
				}
				if (strcmp(buf_vt100, "[F") == 0)
				{
					*scan = line_end;
					break;
				}
				if (strcmp(buf_vt100, "[H") == 0)
				{
					*scan = line_start;
					break;
				}
				if (strcmp(buf_vt100, "[5~") == 0)
				{
					*scan = page_up;
					break;
				}
				if (strcmp(buf_vt100, "[6~") == 0)
				{
					*scan = page_down;
					break;
				}
				if (strcmp(buf_vt100, "[2~") == 0)
				{
					*scan = help;
					break;
				}
				if (strcmp(buf_vt100, "OP") == 0)
				{
					*scan = fk1;
					break;
				}
				if (strcmp(buf_vt100, "OQ") == 0)
				{
					*scan = fk2;
					break;
				}
				if (strcmp(buf_vt100, "OR") == 0)
				{
					*scan = fk3;
					break;
				}
				if (strcmp(buf_vt100, "OS") == 0)
				{
					*scan = fk4;
					break;
				}
...
{
	uart_puts(USART0, "\x1b[5~");
}

void page_down_cb()
{
	uart_puts(USART0, "\x1b[6~");
}
/***************** clock and wait ********************/
volatile uint32_t sys_tick_cnt=0;
#define MAX_COUNT (0x1<<24)
void SysTick_Handler(void)
{
	sys_tick_cnt = (sys_tick_cnt+1)%MAX_COUNT;
}
...
******/
{
	uint32_t start_time = sys_tick_cnt;
	uint32_t end_time = (start_time+(uint32_t)(time))%MAX_COUNT;
	while(sys_tick_cnt<end_time){
...
{
    FRESULT res;
    /* "2:/" si SDDISK vaut 2 (défini par FatFS), déjà préparé par le code */
    const TCHAR driverNumberBuffer[3U] = { SDDISK + '0', ':', '/' };
    /* même chaîne mais sans '/', utile pour f_mount */
    TCHAR drvOnly[3] = { SDDISK + '0', ':', 0 };

    if (!card_ready) {
        /* Init card */
        if (SD_CardInit(&card) != kStatus_Success) {
            return;
        }

        /* mount file system */
        res = f_mount(&fs, drvOnly, 1);        /* 1 = mount now */
        if (res != FR_OK) return;

        /* select the drive */
        res = f_chdrive(driverNumberBuffer);   /* "2:/" */
        if (res != FR_OK) return;
...
char *fs_cd (char *dir)
{
	...
	FRESULT fr;
		...
		fr = f_stat(dir, &fno);
		f_getcwd(cur_path, sizeof(cur_path));
	//	sys_print(cur_path);
		switch (fr) {
		case FR_OK: /* The directory exists */
			if (*dir!=0)
			{
				strcat(cur_path, dir);
				f_chdir(dir);
		...
		case FR_NO_FILE:
		case FR_NO_PATH: /* The directory does not exist */
...
int fs_mkdir(char* dirname)
{
	...
	FRESULT fr;
		...
		switch (fr) {
		case FR_OK: /* The directory exists */
			sprintf(buf, "%s already exists, please find another name.\n", dirname);
			sys_print(buf);
			break;
...
int fs_rm(char* filename)
{
	...
	FRESULT res;
		...
		if (res == FR_OK) sys_print("File successfully removed.");
		if (res == FR_DENIED) sys_print("Problem occurred when trying to remove.");
...
int _open(const char *name, int flags, int mode) {
    (void)mode;                 /* non utilisé avec FatFS */
    if (!card_ready || !name) return -1;

    /* chercher un slot libre */
    int slot = -1;
    for (int i = 0; i < MAX_FILES; i++) {
        if (files[i].obj.fs == NULL) { slot = i; break; }
    }
    if (slot < 0) return -1;
    ...
    switch (flags & O_ACCMODE) {
        case O_WRONLY: amode |= FA_WRITE;             break;
        case O_RDWR:   amode |= FA_READ | FA_WRITE;   break;
        default:       amode |= FA_READ;              break; /* O_RDONLY */
    }

    /* Méthode d'ouverture (création / append / etc.) */
    if (flags & O_APPEND) {
        amode |= FA_OPEN_APPEND;
    } else if (flags & O_TRUNC) {
        amode |= FA_CREATE_ALWAYS;
    } else if (flags & O_CREAT) {
        if (flags & O_EXCL) {
            amode |= FA_CREATE_NEW;
        } else {
            amode |= FA_OPEN_ALWAYS;
        }
    } else {
        amode |= FA_OPEN_EXISTING;
    }

    FRESULT fr = f_open(&files[slot], name, amode);
    if (fr != FR_OK) {
        printf("f_open error: %d (flags=%08x, amode=%02x)\r\n", fr, flags, amode);
        memset(&files[slot], 0, sizeof(FIL));
        return -1;
    }

    return slot + 3; /* 0,1,2 = stdin/out/err → on commence à 3 */
}

int _close(int fd) {
    int slot = fd - 3;
    if (slot < 0 || slot >= MAX_FILES) return -1;
    if (files[slot].obj.fs == NULL)    return -1;

    FRESULT res = f_close(&files[slot]);
    memset(&files[slot], 0, sizeof(FIL));   /* libère le slot */
...
int _read(int fd, char *ptr, int len) {
    ...
    UINT br = 0;
    FRESULT res = f_read(&files[slot], ptr, (UINT)len, &br);
    if (res != FR_OK) {
        sys_print("File could not be opened in read mode.\n");
        return -1;
    }
...
int _write(int fd, char *ptr, int len) {
    ...
    UINT bw = 0;
    FRESULT res = f_write(&files[slot], ptr, (UINT)len, &bw);
    if (res != FR_OK) {
        sys_print("File could not be opened in write mode.\n");
        return -1;
    }

    /* Assure la persistance immédiate sur la carte SD */
    res = f_sync(&files[slot]);
...
static int allocate_free_buffer(void)
{
...
#if ASYNCMODE
    int idx = -1;
    while ((idx = allocate_free_buffer()) == -1) {
    }

    pool[idx].data[0] = (char)(color & 0xFF);
    pool[idx].data[1] = (char)((color >> 8) & 0xFF);

    while (!mb_push_evt(EVT_FORECOLOR | (idx << 16), false));
...
uint32_t lcd_set_alignment(DC *dc, uint32_t alignment)
{
    ...
    pool[idx].data[0] = (alignment & 0xFF);
    pool[idx].data[1] = ((alignment >> 8) & 0xFF);
    pool[idx].data[2] = ((alignment >> 16) & 0xFF);
    pool[idx].data[3] = ((alignment >> 24) & 0xFF);

    while (!mb_push_evt(EVT_SETALIGNMENT | (idx << 16), true));
...
uint32_t lcd_set_direction(DC *dc, uint32_t direction)
{
    ...
    pool[idx].data[0] = (direction & 0xFF);
    pool[idx].data[1] = ((direction >> 8) & 0xFF);
    pool[idx].data[2] = ((direction >> 16) & 0xFF);
    pool[idx].data[3] = ((direction >> 24) & 0xFF);

    while (!mb_push_evt(EVT_SETDIRECTION | (idx << 16), true));

    return direction;

#else

	while (!mb_push_evt(EVT_SETDIRECTION | (uint8_t) direction , false)) ;
	while (!handshake()) {}
	return direction;
...
void lcd_draw_point(uint16_t x, uint16_t y, Color c) {
	  ...
	  pool[idx].data[0] = (char)(x & 0xFF);
	  pool[idx].data[1] = (char)((x >> 8) & 0xFF);
	  pool[idx].data[2] = (char)(y & 0xFF);
	  pool[idx].data[3] = (char)((y >> 8) & 0xFF);
	  while (!mb_push_evt(EVT_DRAWPOINT | (idx << 16), false));


#else
	shdata[0] = (SPoint){x, y};
	lcd_set_forecolor(c);
	while (!mb_push_evt(EVT_DRAWPOINT, false));
	while (!handshake()) {}
...
void lcd_draw_line(int16_t x1, int16_t y1, int16_t x2, int16_t y2, Color c)
{
	...
	pool[idx].data[0] = (char)(x1 & 0xFF);
	pool[idx].data[1] = (char)((x1 >> 8) & 0xFF);
	pool[idx].data[2] = (char)(y1 & 0xFF);
	pool[idx].data[3] = (char)((y1 >> 8) & 0xFF);
	pool[idx].data[4] = (char)(x2 & 0xFF);
	pool[idx].data[5] = (char)((x2 >> 8) & 0xFF);
	pool[idx].data[6] = (char)(y2 & 0xFF);
	pool[idx].data[7] = (char)((y2 >> 8) & 0xFF);
	while (!mb_push_evt(EVT_DRAWLINE | (idx << 16), false));
	...
	while (!mb_push_evt(EVT_DRAWLINE, false)) ;
	while (!handshake()) {}
...
void lcd_draw_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, Color c)
{
	...
	pool[idx].data[2] = (char)(y & 0xFF);
	pool[idx].data[3] = (char)((y >> 8) & 0xFF);
	pool[idx].data[4] = (char)(w & 0xFF);
	pool[idx].data[5] = (char)((w >> 8) & 0xFF);
	pool[idx].data[6] = (char)(h & 0xFF);
	pool[idx].data[7] = (char)((h >> 8) & 0xFF);
	while (!mb_push_evt(EVT_DRAWRECT | (idx << 16), false));
	...
	shdata[1].x = w;
	shdata[1].y = h;
	while (!mb_push_evt(EVT_DRAWRECT, false)) ;
...
void lcd_draw_segments(SPoint *p, int n, Color c)
{
	if (n<MAXPOINTS) {
		lcd_set_forecolor(c);
		while (!mb_push_evt(EVT_DRAWSEGMENTS|((uint16_t)n&0xFFF),false)) ;
		while (!handshake()) {}
	}
}
...
{
	if (n<MAXPOINTS) {
		while (!mb_push_evt(EVT_DRAWLINES|((uint16_t)n&0xFFF),false)) ;
		while (!handshake()) {}
	}
}
...
void lcd_draw_path2d(Graph *g, SPoint *p, int n)
{
	...
	for (int i = 1; i < n; ++i)
	{
		lcd_draw_line(x, y, p[i].x, p[i].y, g->color);
		x = p[i].x;
		y = p[i].y;
	}
}
...
void lcd_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
  ...
  pool[idx].data[6] = (char)(h & 0xFF);
  pool[idx].data[7] = (char)((h >> 8) & 0xFF);
  while (!mb_push_evt(EVT_FILLRECT | (idx << 16), false));
	...
	while (!mb_push_evt(EVT_FILLRECT, false));
	while (!handshake()) {}
...
void lcd_clip(int x1, int y1, int x2, int y2)
{
	...
	pool[idx].data[6] = (char)(y2 & 0xFF);
	pool[idx].data[7] = (char)((y2 >> 8) & 0xFF);
	while (!mb_push_evt(EVT_CLIP | (idx << 16), false));
...
void lcd_draw_string(DC *dc, int16_t x, int16_t y, const char *s)
{
    ...
    int length = strlen(s);
    int nPairs = (length + 1) / 2;

    int idx = -1;
    while ((idx = allocate_free_buffer()) == -1) {
    }

    // pack coordinates first
    pool[idx].data[0] = (uint8_t)(x & 0xFF);
    pool[idx].data[1] = (uint8_t)(x >> 8);
    pool[idx].data[2] = (uint8_t)(y & 0xFF);
    pool[idx].data[3] = (uint8_t)(y >> 8);

    // then the string data: pairs of chars
    int offset = 4;
    for (int i = 0; i < nPairs; ++i) {
        pool[idx].data[offset++] = (uint8_t)s[2 * i];
        if (2 * i + 1 < length)
            pool[idx].data[offset++] = (uint8_t)s[2 * i + 1];
        else
            pool[idx].data[offset++] = 0;
    }

    while (!mb_push_evt(EVT_DRAWSTRING | ((uint32_t)idx << 16) | (uint32_t)(length & 0xFFFF), false));
	...
	for(int i=0; i<length; i+=2){
		shdata[dataIndex].x = (uint8_t)s[i];
		shdata[dataIndex].y = (i + 1 < length) ? (uint8_t)s[i + 1] : 0;
		dataIndex++;
	}
