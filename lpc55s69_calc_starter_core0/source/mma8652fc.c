...
status_t mma8652_read_xyz(int32_t* data)
{
    ...
    int offset = (512/(0x1<<scale));
    int shift = 10 - (int)scale;
	/* A COMPLETER */
    for (int i=0;i<6;i+=2){
    	int32_t word = ((int16_t)buf[i]<<8 | buf[i+1])>>4;
    	if (word > (0x1<<11)) word -= 0x1<<12;
    	int32_t result = (1000*(int32_t)word + offset) >> shift;
    	data[i/2] = result;
    }
