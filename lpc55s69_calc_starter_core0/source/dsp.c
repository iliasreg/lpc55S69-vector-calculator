...
header* maccel (Calc *cc, header *hd)
{
	...
	result = new_matrix(cc,1,3,"accel");
	real *m;
	int32_t data[3];
...
header* mpqcos(Calc* cc, header* hd) {
	...
	if (hd->type == s_real){
		...
		real out;
		PQ_CosF32(in,&out);
		result = new_real(cc,out,"cos");
	}
	else if (hd->type == s_matrix){
		getmatrix(hd,&r,&c,&mIn);
		if (r!=1) cc_error(cc,"row vector expected");
		result = new_matrix(cc,r,c,"matrix_cos");
		mOut = matrixof(result);
		PQ_VectorCosF32(mIn,mOut,(int32_t)c);
	}
	else {
		 cc_error(cc,"real or real array was expected!");
	}
	return pushresults(cc,result);
}
...
header* mpqfft (Calc *cc, header *hd)
{
	...
	real *mIn,*mOut;
	hd = getvalue(cc,hd);
	if (!(hd->type==s_matrix || hd->type==s_real || hd->type==s_cmatrix || hd->type==s_complex)){
		cc_error(cc,"real or complexe array was expected!");
	}
	...
	if (hd->type == s_matrix || hd->type == s_real) {
		...
		getmatrix(hd,&r,&c,&mIn);
		if (r!=1) cc_error(cc,"row vector expected");
		result = new_cmatrix(cc,1,c,"pqfft");
		mOut = matrixof(result);
		PQ_TransformRFFT(POWERQUAD,c,mIn,mOut);
	}
	else {
		getmatrix(hd,&r,&c,&mIn);
		if (r!=1) cc_error(cc,"row vector expected");
		result = new_cmatrix(cc,1,c,"pqfft");
		mOut = matrixof(result);
		PQ_TransformCFFT(POWERQUAD,c,mIn,mOut);
	}
	return pushresults(cc,result);
}
...
header* mpqifft (Calc* cc, header* hd)
{
	...
	result=new_cmatrix(cc,1,c,"");
	mOut=matrixof(result);
	PQ_TransformIFFT(POWERQUAD,c,mIn,mOut);
	return pushresults(cc,result);
}
