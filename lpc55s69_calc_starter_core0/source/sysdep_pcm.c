...
float iir_calc(iir_filter_t *f, float x)
{
	...
	for (int i=0; i< f->num_stages; i++) {
		float *a = bq->a;float *b = bq->b; float *w = bq->w;
...
void pcm_biquad(real *b, real *a, int r, int c, real *n)
{
	...
	for (int i=0; i<r; i++) {
		...
		/* A COMPLETER */
		state[i].param.v_n = 0;
		state[i].param.v_n_1 = 0;
		state[i].param.a_1 = a[3*i+1];
		state[i].param.a_2 = a[3*i+2];
		state[i].param.b_0 = b[3*i];
		state[i].param.b_1 = b[3*i+1];
		state[i].param.b_2 = b[3*i+2];
		state[i].compreg=0;
	}
	*n=(real)pcm_loop(iir_filter_cb);
}
